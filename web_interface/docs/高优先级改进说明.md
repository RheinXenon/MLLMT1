# 高优先级改进说明

本文档记录了针对多图片处理功能的三项高优先级改进。

## 改进日期
2025年11月3日

---

## 改进1：添加并发控制机制 ✅

### 问题
- 多个用户同时上传多张大图可能导致服务器OOM
- 缺少请求队列或限流机制

### 解决方案
使用**信号量(Semaphore)**实现并发控制：

#### 配置文件 (config.py)
```python
# 并发控制配置
MAX_CONCURRENT_REQUESTS = 3  # 最大并发请求数（防止服务器过载）
REQUEST_TIMEOUT = 300  # 请求超时时间（秒）
```

#### 后端实现 (app.py)
1. **创建全局信号量**：
```python
from threading import Semaphore
request_semaphore = Semaphore(config.MAX_CONCURRENT_REQUESTS)
```

2. **装饰器方式**（用于非流式接口）：
```python
def with_concurrency_limit(f):
    """装饰器：限制并发请求数"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if not request_semaphore.acquire(blocking=False):
            return jsonify({"error": "服务器繁忙，请稍后重试"}), 429
        try:
            return f(*args, **kwargs)
        finally:
            request_semaphore.release()
    return decorated_function

@app.route('/api/chat', methods=['POST'])
@with_concurrency_limit  # 应用装饰器
def chat():
    # ... 处理逻辑
```

3. **手动控制方式**（用于流式接口）：
```python
@app.route('/api/chat_stream', methods=['POST'])
def chat_stream():
    # 获取信号量
    if not request_semaphore.acquire(blocking=False):
        return error_response("服务器繁忙")
    
    def generate():
        try:
            # ... 生成逻辑
        finally:
            request_semaphore.release()  # 在finally中释放
```

### 效果
- ✅ 限制最大并发数为3个请求
- ✅ 超出限制时返回429状态码（Too Many Requests）
- ✅ 防止服务器过载和OOM

---

## 改进2：统一图片压缩逻辑 ✅

### 问题
- 图片压缩逻辑不统一
- 非流式接口没有调用图片压缩
- 代码重复

### 解决方案
在模型管理器的响应生成方法中**统一预处理图片**：

#### 模型管理器 (model_manager.py)

**非流式输出**：
```python
def generate_response_with_history(self, prompt, image_paths=None, ...):
    # 记录压缩后的图片路径
    compressed_paths = []
    
    try:
        # 统一预处理图片（压缩以节省显存）
        if image_paths and len(image_paths) > 0:
            logger.info("🖼️ 开始预处理图片...")
            processed_paths = []
            for img_path in image_paths:
                processed_path = self.preprocess_image(img_path, max_size=1024)
                processed_paths.append(processed_path)
                # 如果生成了压缩文件，记录下来
                if processed_path != img_path:
                    compressed_paths.append(processed_path)
            image_paths = processed_paths
        
        # 清理CUDA缓存
        self.clear_cuda_cache()
        
        # ... 生成逻辑 ...
        
        return {
            "success": True,
            "response": response,
            "compressed_paths": compressed_paths  # 返回压缩文件路径
        }
```

**流式输出**：
```python
def generate_response_stream(self, prompt, image_paths=None, ..., 
                            compressed_paths_container=None):
    """
    compressed_paths_container: 用于返回压缩文件路径的列表容器
    """
    try:
        # 统一预处理图片
        if image_paths and len(image_paths) > 0:
            processed_paths = []
            for img_path in image_paths:
                processed_path = self.preprocess_image(img_path, max_size=1024)
                processed_paths.append(processed_path)
                # 记录到容器
                if processed_path != img_path and compressed_paths_container is not None:
                    compressed_paths_container.append(processed_path)
            image_paths = processed_paths
        
        # ... 流式生成逻辑 ...
```

### 效果
- ✅ 所有路径都统一压缩图片
- ✅ 消除代码重复
- ✅ 节省显存占用
- ✅ 压缩文件路径可追踪

---

## 改进3：清理压缩后的临时文件 ✅

### 问题
- 压缩后的临时文件（`*_compressed.*`）没有被清理
- uploads目录积累大量临时文件
- 可能导致磁盘空间不足

### 解决方案
扩展清理逻辑，清理**原始文件和压缩文件**：

#### 后端实现 (app.py)

**非流式接口**：
```python
@app.route('/api/chat', methods=['POST'])
def chat():
    # ... 生成回复 ...
    result = model_manager.generate_response_with_history(...)
    
    # 获取压缩文件路径
    compressed_paths = result.get('compressed_paths', [])
    
    # 清理所有临时文件
    all_temp_files = image_paths + compressed_paths
    for temp_path in all_temp_files:
        if os.path.exists(temp_path):
            try:
                os.remove(temp_path)
                logger.info(f"删除临时文件: {temp_path}")
            except Exception as e:
                logger.warning(f"删除临时文件失败: {e}")
```

**流式接口**：
```python
@app.route('/api/chat_stream', methods=['POST'])
def chat_stream():
    def generate():
        # 用于接收压缩文件路径的容器
        compressed_paths = []
        
        try:
            # 流式生成（传递容器）
            for chunk in model_manager.generate_response_stream(
                ...,
                compressed_paths_container=compressed_paths
            ):
                yield chunk
        finally:
            # 清理所有临时文件
            all_temp_files = image_paths + compressed_paths
            logger.info(f"清理{len(all_temp_files)}个临时文件（原始:{len(image_paths)}, 压缩:{len(compressed_paths)}）")
            for temp_path in all_temp_files:
                if os.path.exists(temp_path):
                    os.remove(temp_path)
```

### 效果
- ✅ 完整清理原始文件和压缩文件
- ✅ 防止临时文件堆积
- ✅ 节省磁盘空间
- ✅ 详细的日志记录

---

## 测试建议

### 1. 并发控制测试
```python
# 使用多线程同时发送4个请求（超过限制3个）
import threading
import requests

def send_request():
    response = requests.post('http://localhost:5000/api/chat_stream', ...)
    print(f"状态码: {response.status_code}")

threads = [threading.Thread(target=send_request) for _ in range(4)]
for t in threads:
    t.start()
for t in threads:
    t.join()

# 预期：3个请求成功处理，1个返回429
```

### 2. 图片压缩测试
```bash
# 查看日志，确认压缩流程
# 应该看到：
# - "🖼️ 开始预处理图片..."
# - "✅ 图片预处理完成，生成了X个压缩文件"
```

### 3. 文件清理测试
```bash
# 监控uploads目录
watch -n 1 "ls -lh web_interface/uploads/"

# 发送带图片的请求后，检查：
# - 原始图片文件
# - 压缩图片文件 (*_compressed.*)
# 都应该在请求完成后被删除
```

---

## 性能指标

### 改进前
- 并发请求：无限制 → **易OOM**
- 图片压缩：不一致 → **显存浪费**
- 临时文件：仅清理原始 → **磁盘堆积**

### 改进后
- 并发请求：最多3个 → **稳定可靠** ✅
- 图片压缩：统一处理 → **显存优化** ✅
- 临时文件：完整清理 → **磁盘干净** ✅

---

## 配置参数

可以根据服务器配置调整以下参数：

| 参数 | 位置 | 默认值 | 说明 |
|------|------|--------|------|
| `MAX_CONCURRENT_REQUESTS` | config.py | 3 | 最大并发请求数 |
| `IMAGE_COMPRESSION_MAX_SIZE` | config.py | 1024 | 图片压缩最大边长 |
| `MAX_PIXELS` | config.py | 1003520 | 模型处理的最大像素数 |

### 调整建议
- **16GB显存**：可提高到 `MAX_CONCURRENT_REQUESTS = 5`
- **32GB显存**：可提高到 `MAX_CONCURRENT_REQUESTS = 8`
- **高分辨率需求**：提高 `IMAGE_COMPRESSION_MAX_SIZE = 2048`

---

## 总结

这三项高优先级改进使系统更加**健壮、高效、可靠**：

1. ✅ **并发控制** - 防止服务器过载
2. ✅ **统一压缩** - 优化显存使用
3. ✅ **完整清理** - 避免磁盘堆积

所有改进都遵循业界最佳实践，代码质量高，易于维护和扩展。

